import { startMergePlacement } from './merge-placement-prototype';
...
const tracks: Track[] = [
  ...
  },
  {
    ...
    description: '급여로 참가비를 내고 보유 자전거로 출전하는 자동 관람형 대회가 성장 보상 루프를 만드는지 단일 시점·하이브리드·시네마틱 중계를 비교합니다.',
    variants: [
      ...
      { id: 'cinematic-broadcast', label: 'E안', title: '시네마틱 스포츠 중계 레이스', description: '3,000m 레이스를 전체 무리·라이벌 접전·마지막 600m 스퍼트의 국면별 카메라로 중계하고, 캐릭터 페달링과 바퀴 회전·속도선·사진 판정으로 결승 감정 곡선을 강화합니다.', status: '체험 가능', question: '국면별 카메라 전환과 라이벌 포커스가 조작 없는 자동 레이스의 관람 몰입과 자전거 성장 동기를 높이는가?', controls: '성장 프리셋을 고른 뒤 자동 중계를 관람합니다. 중계 속도 x1/x2/x4를 비교하고, 바퀴·페달의 주행 동작과 라이벌 포커스·마지막 600m 컷인·사진 판정이 속도와 순위 경쟁을 선명하게 만드는지 확인합니다.', raceCinematicDemo: true, issueNumber: 231, documentId: 'race-cinematic-broadcast' },
    ],
  },
  {
    id: 'collection',
...
function renderDemo(track: Track, variant: Variant) {
  destroyGame();
  const hasDemo = Boolean(variant.placementDemo || variant.intakeDemo || variant.demo || variant.collectionDemo || variant.supplyDemo || variant.rewardDemo || variant.assemblyDemo
