...
게임 시작 전에 계정 세션을 만들고, 계정별 게임 프로필과 진행을 분리한다. 각 플레이 세션은 5분짜리 Day로 묶으며 실제 플레이 화면이 활성 상태일 때만 시간을 차감한다. 시간 종료 뒤에는 정산을 거쳐 다음 Day를 명시적으로 시작한다.
...
2. 플레이 화면에서만 남은 시간이 감소하고 백그라운드에서는 감소하지 않는다.
3. 시간 종료와 `DAY 종료`를 반복 호출해도 같은 Day 정산 보상이 두 번 적용되지 않는다.
4. 계정 A 진행 후 계정 B로 전환하면 초기 슬롯이 보이고, A 재로그인 시 A의 진행만 복원된다.
5. 진행 중 새로고침 후 같은 Day 번호·남은 시간·재화·주문·정산 이력이 복원된다.
6. 다음 Day 시작 뒤 이전 정산 이력은 유지되고 현재 Day만 새 준비 상태가 된다.
