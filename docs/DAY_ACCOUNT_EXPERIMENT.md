...
Day 진행 중 자전거 한 대를 완성하면 별도 보상 화면으로 이동하지 않는다. 납품 보상과 오늘 수입은 완성 시점에 즉시 반영하고(다음 주문 전환 연출만 지연), 남은 보드 부품을 유지한 채 다음 자전거 주문을 바로 시작한다.
...
- 화면 이동(game·guide를 벗어나면 일시정지), 로그아웃, 컨트롤러 종료(일시정지 후 저장)
- 저장 실패(용량 초과·프라이빗 모드)는 예외 없이 메모리 진행을 유지하고 Lab 셸 라벨에 경고를 표시한다. 복구 시 손상된 정산 이력 요소는 버리고 숫자 필드는 보정한다.
...
2. 홈 화면에는 현재 일차가 보이고, 플레이의 390×810 캔버스 내부에는 현재 일차·큰 10초 타이머·남은 시간 막대·오늘 수입이 함께 보인다. Lab 외곽 헤더에는 동일 정보를 중복 표시하지 않는다.
