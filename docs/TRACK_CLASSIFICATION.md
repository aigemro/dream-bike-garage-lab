...
기존 `Game Core`는 단일 트랙이 아니라 여러 실험 트랙을 담는 상위 범주였습니다. MVP 핵심 루프와 장기 성장, 표현, 기술 기반, 통합 검증의 평가 질문이 서로 달라 다음 다섯 영역으로 나눕니다.
...
| Art & Audio | 동일한 게임 상태를 전달하는 시각·청각 표현 | 게임 규칙과 판정 |
...
조립 트랙은 장착 시점과 완료 판정을 정의하고, Art & Audio는 그 상태를 어떻게 보이고 들려줄지 비교합니다.
...
`MVP 핵심 루프 통합`은 다음 흐름의 상태 전달과 완주 여부만 담당합니다.
...
3. 동일한 상태의 보임·들림만 바꾸면 `Art & Audio`에 둡니다.
4. 여러 게임 기능이 공유하는 실행 환경 문제면 `플레이 기반`에 둡니다.
5. 두 개 이상 선택안 사이의 상태 연결과 출시 완주를 검증하면 `릴리스 통합`에 둡니다.
