...
Issue #114의 통합 트랙 **MVP 핵심 기능 통합**은 별도의 축약형 슬라이스를 만들지 않고, 실제 머지 코어 C안 화면(`merge-prototype.ts`의 `integrated` 모드) 위에 다음을 직접 연결합니다.

- 부품 수급: 카테고리 선택형 택배 (주문 → 배송 → 개봉 → C안 보드 배치·회전)
- 주문 조립: 목표 레벨 부품 완성 시 보드에서 소비하고 부품별 자동 장착 연출(#151 방식) 후 주문 완료 전환

초기 1차 수직 슬라이스(PR #155)의 독립 DOM 보드 접근은 C안 보드를 대체한다는 문제로 폐기했고, 급여·드림 바이크 성장·로컬 저장은 이번 범위에서 제외합니다.

이번 구현은 기능 계약 검증이 목적이므로 화면 디자인, 아트 스타일, 자전거 모델 고도화, 고품질 장착 애니메이션과 사운드는 제외합니다. 관련 표현은 기존 C안 컴포넌트와 텍스트 상태 표시만 사용합니다.
